...
const LockerGrid = () => {
  ...
  const fetchLockerStatus = async (start: Date, end: Date) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/api/reservation/status?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
      );
  ...
  const reserveLocker = async (lockerId: number) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8080/api/reservation`, reservationRequest);
